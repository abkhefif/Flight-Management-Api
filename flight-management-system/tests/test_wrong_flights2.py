...
def test_create_wrong_flight_2(client, api_url):
    airport_1 = {
    ...
    flight_2 = {
            ...
            "departure_airport_id" : depart_airport["id"],
            "arrival_airport_id" : ariv_airport["id"],
            "scheduled_departure": (now + timedelta(hours=2,minutes=5)).isoformat(),
            "scheduled_arrival": (now + timedelta(hours=5)).isoformat(),
            "status" : "SCHEDULED"
            }
    response_flight_2 = client.post(f"{api_url}/flights", json = flight_2)
    print(response_flight.json())
    assert response_flight_2.status_code == 409
    flight = response_flight.json()
